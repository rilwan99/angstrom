...
use jsonrpsee_core::{
    id_providers::RandomIntegerIdProvider, server::Methods, traits::IdProvider, Error,
    TEN_MB_SIZE_BYTES
};
...
use crate::{
    future::{ConnectionGuard, ServerHandle, StopHandle},
    ...
    <B as HttpBody>::Data: Send
{
    ...
    ///
    /// This will run on the tokio runtime until the server is stopped or the
    ...
    async fn start_inner(self, methods: Methods, stop_handle: StopHandle) {
        ...
        // Once this channel is closed it is safe to assume that all connections have
        // been gracefully shutdown
        while process_connection_awaiter.recv().await.is_some() {
            // Generally, messages should not be sent across this channel,
            // but we'll loop here to wait for `None` just to be on the safe
            // side
        }
    }
}
...
struct Settings {
    ...
    /// Enable WS.
    enable_ws: bool,
    /// Number of messages that server is allowed to `buffer` until backpressure
    /// kicks in.
...
pub enum BatchRequestConfig {
    ...
    Disabled,
    /// Each batch request is limited to `len` and any batch request bigger than
    /// `len` will not be processed.
    Limit(u32),
...
impl<B, L> Builder<B, L> {
    ...
    /// by the server.
    ///
    /// Default: batch requests are allowed and can be arbitrary big but the
    /// maximum payload size is limited.
    ...
    /// [`Logger`](../jsonrpsee_core/logger/trait.Logger.html).
    ///
    /// ```
    ...
    ///
    /// This option is used to keep the connection alive, and is just submitting
    /// `Ping` frames, without making any assumptions about when a `Pong`
    ...
    /// Configure a custom [`tower::ServiceBuilder`] middleware for composing
    /// layers to be applied to the RPC service.
    ///
    ...
    ///
    /// That implies that server just denies HTTP requests which isn't a
    ...
    /// Finalizes the configuration of the server with customized TCP settings
    /// on the socket.
    ///
    ///
    /// ```rust
...
pub(crate) struct ServiceData<L: Logger> {
    ...
    /// Number of messages that server is allowed `buffer` until backpressure
    /// kicks in.
...
impl<L: Logger> hyper::service::Service<hyper::Request<hyper::Body>> for TowerService<L> {
    // The following associated type is required by the `impl<B, U, L: Logger>
    // Server<B, L>` bounds. It satisfies the server's bounds when the
    // `tower::ServiceBuilder<B>` is not set (ie `B: Identity`).
    ...
    fn call(&mut self, request: hyper::Request<hyper::Body>) -> Self::Future {
        ...
        if self.inner.enable_ws && is_upgrade_request {
            ...
            let response = match server.receive_request(&request) {
                ...
                Err(e) => {
                    ...
                    hyper::Response::new(hyper::Body::from(format!(
                        "Could not upgrade connection: {e}"
                    )))
                }
            };
            ...
            // The request wasn't an upgrade request; let's treat it as a standard HTTP
            // request:
    ...
    <U as HttpBody>::Data: Send
{
    ...
    tokio::spawn(async {
        to_http_service(socket, service, cfg.stop_handle)
            .in_current_span()
            .await;
        drop(drop_on_completion)
    });
