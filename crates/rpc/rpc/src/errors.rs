...
pub enum WsHttpSamePortError {
    ...
    /// domains.
    #[error(
        "CORS domains for http and ws are different, but they are on the same port: http: \
         {http_cors_domains:?}, ws: {ws_cors_domains:?}"
    )]
    ...
    #[error(
        "Different api modules for http and ws on the same port is currently not supported: http: \
         {http_modules:?}, ws: {ws_modules:?}"
    )]
    ConflictingModules {
        /// Http modules.
        http_modules: Vec<GuardRpcModule>,
        /// Ws modules.
        ws_modules:   Vec<GuardRpcModule>
    }
}
