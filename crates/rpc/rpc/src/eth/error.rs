...
use reth_transaction_pool::error::{
    Eip4844PoolTransactionError, InvalidPoolTransactionError, PoolError, PoolOrderError
};
...
pub enum EthApiError {
    ...
    /// Thrown when constructing an RPC block from a primitive block data
    /// failed.
    #[error(transparent)]
    InvalidBlockData(#[from] BlockError),
    /// Thrown when a [AccountOverride](reth_rpc_types::state::AccountOverride)
    /// contains conflicting `state` and `stateDiff` fields
    ...
    /// Thrown when a transaction was requested but not matching transaction
    /// exists
...
pub enum RpcInvalidTransactionError {
    ...
    GasUintOverflow,
    /// returned if the transaction is specified to use less gas than required
    /// to start the invocation.
    ...
    FeeCapVeryHigh,
    /// Thrown post London if the transaction's fee is less than the base fee of
    /// the block
    ...
    MemoryOutOfGas(U256),
    /// As BasicOutOfGas but thrown when gas exhausts during precompiled
    /// contract execution.
    ...
    /// `blob_hashes`/`blob_versioned_hashes` is not supported for blocks before
    /// the Cancun hardfork.
    #[error("blob_versioned_hashes is not supported for blocks before the Cancun hardfork.")]
    BlobVersionedHashesNotSupported,
    /// Block `blob_gas_price` is greater than tx-specified
...
impl From<reth_primitives::InvalidTransactionError> for RpcInvalidTransactionError {
    fn from(err: reth_primitives::InvalidTransactionError) -> Self {
        ...
        // inside the txpool (e.g. `eth_sendRawTransaction`) to their
        // corresponding RPC
        match err {
...
pub enum RpcPoolError {
    ...
    /// Custom pool error
    #[error("{0:?}")]
    PoolOrderError(Box<dyn PoolOrderError>),
...
impl From<PoolError> for RpcPoolError {
    fn from(err: PoolError) -> RpcPoolError {
        match err {
            ...
            PoolError::FeeCapBelowMinimumProtocolFeeCap(..) => RpcPoolError::Underpriced,
            PoolError::SpammerExceededCapacity(..) => RpcPoolError::TxPoolOverflow,
...
impl From<InvalidPoolTransactionError> for RpcPoolError {
    fn from(err: InvalidPoolTransactionError) -> RpcPoolError {
        match err {
            ...
            InvalidPoolTransactionError::ExceedsGasLimit(..) => RpcPoolError::ExceedsGasLimit,
            ...
            InvalidPoolTransactionError::OversizedData(..) => RpcPoolError::OversizedData,
...
/// Converts the evm [ExecutionResult] into a result where `Ok` variant is the
/// output bytes if it is [ExecutionResult::Success].
