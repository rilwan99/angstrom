    ...
    Network: Send + Sync + 'static
{
    ...
    /// Returns the number of transactions sent from an address at the given
    /// block identifier.
    ///
    /// If this is [BlockNumberOrTag::Pending] then this will look up the
    /// highest transaction in pool and return the next nonce (highest + 1).
    ...
    pub(crate) async fn get_proof(
        ...
        // if we are trying to create a proof for the latest block, but have a BlockId
        // as input that is not BlockNumberOrTag::Latest, then we need to figure
        // out whether or not the BlockId corresponds to the latest block
