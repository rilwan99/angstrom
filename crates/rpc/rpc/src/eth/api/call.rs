//! Contains RPC handler implementations specific to endpoints that call/execute
//! within evm.
    ...
    Network: NetworkInfo + Send + Sync + 'static
{
    ...
    pub async fn call_many(
        ...
        // we're essentially replaying the transactions in the block here, hence we need
        // the state that points to the beginning of the block, which is the
    ...
    ///
    /// This will execute the [CallRequest] and find the best gas limit via
    /// binary search
        ...
        S: StateProvider
    {
        ...
        // get the highest possible gas limit, either the request's set value or the
        // currently configured gas limit
        ...
        // Exceptional case: init used too much gas, we need to increase the gas limit
        // and try again
        ...
        // at this point we know the call succeeded but want to find the _best_ (lowest)
        // gas the transaction succeeds with. we  find this by doing a binary
        // search over the possible range NOTE: this is the gas the transaction
        // used, which is less than the transaction requires to succeed
    ...
    async fn create_access_list_with(
        ...
        // we want to disable this in eth_createAccessList, since this is common
        // practice used by other node impls and providers <https://github.com/foundry-rs/foundry/issues/4388>
...
/// Executes the requests again after an out of gas error to check if the error
/// is gas related or not
#[inline]
