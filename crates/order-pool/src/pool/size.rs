...
///
/// Note: We do not assume that size tracking is always exact. Depending on the
/// bookkeeping of the additions and subtractions the total size might be
/// slightly off. Therefore, the underlying value is an `isize`, so that the
/// value does not wrap.
