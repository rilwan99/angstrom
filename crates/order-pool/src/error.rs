...
pub enum PoolError {
    ...
    /// Thrown if a replacement transaction's gas price is below the already
    /// imported transaction
    #[error("[{0:?}]: insufficient gas price to replace existing transaction.")]
    ReplacementUnderpriced(TxHash),
    /// The fee cap of the transaction is below the minimum fee cap determined
    /// by the protocol
    #[error("[{0:?}] Transaction feeCap {1} below chain minimum.")]
    FeeCapBelowMinimumProtocolFeeCap(TxHash, u128),
    /// Thrown when the number of unique transactions of a sender exceeded the
    /// slot capacity.
    #[error("{0:?} identified as spammer. Transaction {1:?} rejected.")]
    SpammerExceededCapacity(Address, TxHash),
    /// Thrown when a new transaction is added to the pool, but then immediately
    /// discarded to respect the size limits of the pool.
    ...
    /// Any other error that occurred while inserting/validating a transaction.
    /// e.g. IO database error
    #[error("[{0:?}] {1:?}")]
...
impl PoolError {
    ...
    ///
    /// This function returns true only if the transaction will never make it
    /// into the pool because its composition is invalid and the original
    /// sender should have detected this as well. This is used to determine
    /// whether the original sender should be penalized for sending an
    ...
    pub fn is_bad_transaction(&self) -> bool {
        match self {
            ...
            PoolError::FeeCapBelowMinimumProtocolFeeCap(..) => {
                ...
                // although this transaction will always be invalid, we do not want to penalize
                // the sender because this check simply could not be implemented
                // by the client
                false
            }
            PoolError::SpammerExceededCapacity(..) => {
                // the sender exceeded the slot capacity, we should not penalize the peer for
                // sending the tx because we don't know if all the transactions are sent from
                // the same peer, there's also a chance that old transactions
                // haven't been cleared yet (pool lags behind) and old
                // transaction still occupy a slot in the pool
                false
            }
            ...
            PoolError::ExistingConflictingTransactionType(..) => {
                // this is not a protocol error but an implementation error since the pool
                // enforces exclusivity (blob vs normal tx) for all senders
                false
            }
        }
    }
...
/// Represents all errors that can happen when validating transactions for the
/// pool for EIP-4844 transactions
#[derive(Debug, thiserror::Error)]
pub enum Eip4844PoolTransactionError {
    /// Thrown if we're unable to find the blob for a transaction that was
    ...
    /// EIP-4844 transactions are only accepted if they're gapless, meaning the
    /// previous nonce of the transaction (`tx.nonce -1`) must either be in
    /// the pool or match the on chain nonce of the sender.
    ///
    /// This error is thrown on validation if a valid blob transaction arrives
    /// with a nonce that would introduce gap in the nonce sequence.
...
pub enum InvalidPoolTransactionError {
    ...
    /// Thrown when a new transaction is added to the pool, but then immediately
    /// discarded to respect the max_init_code_size.
    ...
    /// than some meaningful limit a user might use. This is not a consensus
    /// error making the transaction invalid, rather a DOS protection.
    ...
    fn is_bad_transaction(&self) -> bool {
        match self {
            InvalidPoolTransactionError::Consensus(err) => {
                ...
                // not be assumed to have been intentionally caused by the
                // sender
                match err {
