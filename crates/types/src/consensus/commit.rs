...
use crate::primitive::BLSSignature;
...
impl Commit {
    ...
    /// Get a reference to the validator bitmap for this Commit.  All the validator maps should always be the same so
    /// we return the one from `message_sig`
    ...
    pub fn add_signature(&mut self, validator_id: BLSValidatorID, sk: &SecretKey<Bls12381G1Impl>) -> bool {
        // These can only fail if the SK is zero in which case they'll all fail so no need to return early
        self.message_sig.sign_into(validator_id, sk, self.hash_message().as_slice());
        self.vanilla_bundle_sig.sign_into(validator_id, sk, self.vanilla_bundle_hash.as_slice());
        self.lower_bound_sig.sign_into(validator_id, sk, self.lower_bound_hash.as_slice());
        self.order_buffer_sig.sign_into(validator_id, sk, self.order_buffer_hash.as_slice())
    }

    pub fn validate(&self, public_key_library: &[PublicKey<Bls12381G1Impl>]) -> bool {
        self.message_sig.validate(public_key_library, self.hash_message().as_slice()) &&
        self.vanilla_bundle_sig.validate(public_key_library, self.vanilla_bundle_hash.as_slice()) &&
        self.lower_bound_sig.validate(public_key_library, self.lower_bound_hash.as_slice()) &&
        self.order_buffer_sig.validate(public_key_library, self.order_buffer_hash.as_slice())
    }

    /// Validate that this commit message is associated with a specific Proposal - incomplete
    ...
    /// Returns true if this Commit claims to have been signed by the specified validator.  This does
    /// not inherently validate the Commit so make sure to do that as well!
    ...
    pub fn from_proposal(proposal: &Proposal, sk: &SecretKey<Bls12381G1Impl>) -> Self {
        ...
        Self::generate_commit_all(block_height, vanilla_bundle_hash, lower_bound_hash, order_buffer_hash, sk) 
    }
}
