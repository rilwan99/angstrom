...
///
/// This helper type's sole purpose is to give the caller more control over from
/// where blocks can be fetched.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Default)]
pub enum BlockSource {
    /// Check all available sources.
    ///
    /// Note: it's expected that looking up pending blocks is faster than
    /// looking up blocks in the database so this prioritizes Pending >
    /// Database.
    #[default]
    Any,
    /// The block was fetched from the pending block source, the blockchain tree
    /// that buffers blocks that are not yet finalized.
    Pending,
...
///
/// If not requested otherwise, implementers of this trait should prioritize
/// fetching blocks from the database.
    ...
    + Sync
{
    ...
    ///
    /// Note: this only operates on the hash because the number might be
    /// ambiguous.
    ///
    ...
    ///
    /// Note: This returns a [SealedBlock] because it's expected that this is
    /// sealed by the provider and the caller does not know the hash.
...
/// Trait extension for `BlockReader`, for types that implement `BlockId`
/// conversion.
///
/// The `BlockReader` trait should be implemented on types that can retrieve a
/// block from either a block number or hash. However, it might be desirable to
/// fetch a block from a `BlockId` type, which can be a number, hash, or tag
...
/// to resolve `BlockId`s to block numbers or hashes, and retrieving the block
/// should be done using the type's `BlockReader` methods.
#[auto_impl::auto_impl(&, Arc)]
pub trait BlockReaderIdExt: BlockReader + BlockIdReader + ReceiptProviderIdExt {
    ...
    ///
    /// Note: This returns a [SealedHeader] because it's expected that this is
    /// sealed by the provider and the caller does not know the hash.
    ...
    fn ommers_by_number_or_tag(&self, id: BlockNumberOrTag) -> Result<Option<Vec<Header>>> {
        self.convert_block_number(id)?
            .map_or_else(|| Ok(None), |num| self.ommers(num.into()))
    }
...
pub trait BlockWriter: Send + Sync {
    /// Insert full block and make it canonical. Parent tx num and transition id
    /// is taken from parent block in database.
    ///
    /// Return [StoredBlockBodyIndices] that contains indices of the first and
    /// last transactions and transition in the block.
    fn insert_block(
        &self,
        block: SealedBlock,
    ...
    /// Appends a batch of sealed blocks to the blockchain, including sender
    /// information, and updates the post-state.
    ///
