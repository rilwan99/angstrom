...
macro_rules! decode {
    ($v:expr) => {
        $v.map_err(|e| $crate::DatabaseError::Read(e.into()))?
            .map(decoder::<T>)
            .transpose()
    };
}

/// Some types don't support compression (eg. H256), and we don't want to be
/// copying them to the allocated buffer when we can just use their reference.
...
impl<'tx, T: Table> DbCursorRW<'tx, T> for Cursor<'tx, RW, T> {
    ...
    fn append(&mut self, key: T::Key, value: T::Value) -> Result<(), DatabaseError> {
        let key = key.encode();
        self.inner
            .put(key.as_ref(), compress_or_ref!(self, value), WriteFlags::APPEND)
            .map_err(|e| DatabaseError::Write {
                code:       e.into(),
...
impl<'tx, T: DupSort> DbDupCursorRW<'tx, T> for Cursor<'tx, RW, T> {
    ...
    fn append_dup(&mut self, key: T::Key, value: T::Value) -> Result<(), DatabaseError> {
        let key = key.encode();
        self.inner
            .put(key.as_ref(), compress_or_ref!(self, value), WriteFlags::APPEND_DUP)
            .map_err(|e| DatabaseError::Write {
                code:       e.into(),
