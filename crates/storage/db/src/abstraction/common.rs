...
///
/// The `Result` represents that the operation might fail, while the `Option`
/// represents whether or not the entry exists.
pub type PairResult<T> = Result<Option<KeyValue<T>>, DatabaseError>;

/// A key-value pair coming from an iterator.
///
/// The `Result` represents that the operation might fail, while the `Option`
/// represents whether or not there is another entry.
