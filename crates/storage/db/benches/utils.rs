...
/// Returns bench vectors in the format: `Vec<(Key, EncodedKey, Value,
/// CompressedValue)>`.
#[allow(unused)]
    ...
    T::Value: Default + Clone + for<'de> serde::Deserialize<'de>
{
    let list: Vec<TableRow<T>> = serde_json::from_reader(std::io::BufReader::new(
        ...
        .expect(
            "Test vectors not found. They can be generated from the workspace by calling `cargo \
             run --bin reth -- test-vectors tables`."
        )
    ))
    .unwrap();
