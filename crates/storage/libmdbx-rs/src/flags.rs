...
pub enum SyncMode {
    ...
    /// the last committed transaction. I.e. it preserves the ACI
    ...
    /// MVCC snapshots corresponding to previous "steady" transactions (see
    /// below).
    ///
    /// With [crate::WriteMap] the [SyncMode::SafeNoSync] instructs MDBX to use
    /// asynchronous mmap-flushes to disk. Asynchronous mmap-flushes means
    /// that actually all writes will scheduled and performed by operation
    /// system on it own manner, i.e. unordered. MDBX itself just notify
    /// operating system that it would be nice to write data to disk, but no
    /// more.
    ///
    ...
    /// [WriteMap](crate::WriteMap) and [EnvironmentFlags::liforeclaim] flags
    /// are not used, then a system crash can't corrupt the database, but
    /// you can lose the last transactions, if at least one buffer is not
    /// yet flushed to disk. The risk is governed by how often the system
    /// flushes dirty buffers to disk and how often
    /// [Environment::sync()](crate::Environment::sync) is called. So,
    ...
    /// typically) or [WriteMap](crate::WriteMap) or
    /// [EnvironmentFlags::liforeclaim] flags are used, you should
