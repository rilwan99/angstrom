...
pub struct PendingSessionHandle {
    /// Can be used to tell the session to disconnect the connection/abort the
    /// handshake process.
...
///
/// Within an active session that supports the `Ethereum Wire Protocol `, three
...
impl ActiveSessionHandle {
    ...
    /// Sends a disconnect command to the session, awaiting the command channel
    /// for available capacity.
    pub async fn try_disconnect(
        &self,
        reason: Option<DisconnectReason>
    ) -> Result<(), SendError<SessionCommand>> {
        self.commands_to_session
            .clone()
...
pub enum PendingSessionEvent {
    ...
    Established {
        ...
        status:       Status,
        /// The actual connection stream which can be used to send and receive
