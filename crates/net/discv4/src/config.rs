...
impl Default for Discv4Config {
    fn default() -> Self {
        Self {
            ...
            /// This should be high enough to cover an entire recursive FindNode
            /// lookup which is includes sending FindNode to nodes
            /// it discovered in the rounds using the concurrency
            /// factor ALPHA
            udp_egress_message_buffer:  1024,
            /// Every outgoing request will eventually lead to an incoming
            /// response
            udp_ingress_message_buffer: 1024,
            max_find_node_failures:     5,
            ping_interval:              Duration::from_secs(60 * 10),
            /// unified expiration and timeout durations, mirrors geth's
            /// `expiration` duration
