...
impl SqrtPriceX96 {
    /// Uses malachite.rs to approximate this value as a floating point number.  Converts from the
    /// internal U160 representation of `sqrt(P)` to an approximated f64 representation of `P`, which
    /// is a change to the value of this number and why this isn't `From<SqrtPriceX96> for f64`
...
/// but may include an arbitrary number of ticks.
#[derive(Debug, Clone)]
...
impl PoolRange {
    pub fn new(lower_tick: Tick, upper_tick: Tick, liquidity: u128) -> Result<Self, String> {
        ...
        if upper_tick <= lower_tick {
            return Err(format!("Upper tick bound less than or equal to lower tick bound for range ({}, {})", lower_tick, upper_tick));
        }
        if upper_tick > MAX_TICK { return Err(format!("Proposed upper tick '{}' out of valid tick range", upper_tick)); }
        if lower_tick < MIN_TICK { return Err(format!("Proposed lower tick '{}' out of valid tick range", lower_tick)); }
...
pub struct MarketSnapshot {
    ...
    /// The current SqrtPriceX96 for this pairing as of this snapshot (𝛥Token1/𝛥Token0)
    sqrt_price_x96: SqrtPriceX96,
    /// The current tick our price lives in - price might not be precisely on a tick bound, this is the LOWER of the possible ticks
    current_tick: Tick,
    /// Index into the 'ranges' vector for the PoolRange that includes the tick our current price lives at/in
    cur_tick_idx: usize,
}
...
impl MarketSnapshot {
    pub fn new(mut ranges: Vec<PoolRange>, sqrt_price_x96: SqrtPriceX96) -> Result<Self, String> {
        ...
        if !ranges.windows(2).all(|w| w[0].upper_tick == w[1].lower_tick) {
        ...
        let Some(cur_tick_idx) = ranges.iter().position(|r| r.lower_tick <= current_tick && current_tick < r.upper_tick) else {
            return Err(format!("Unable to find initialized tick window for tick '{}'", current_tick));
        };
    ...
    pub fn get_range_for_tick(&self, tick: Tick) -> Option<(usize, &PoolRange)> {
        let range_idx = self.ranges.iter().position(|r| r.lower_tick <= tick && tick < r.upper_tick);
    ...
    pub fn current_position(&self) -> MarketPrice {
        MarketPrice { state: self, range_idx: self.cur_tick_idx, tick: self.current_tick, price: self.sqrt_price_x96 }
    }
}

/// A MarketPrice represents a price based on the market state preserved in a parent MarketSnapshot.  The
/// MarketPrice can be moved and operated on to simulate the behavior of the price if the underlying assets
...
impl<'a> MarketPrice<'a> {
    /// Build a PriceRange from the current price to the closer of either the target price or the bound of
    /// the current liquidity pool.  Will jump to the next liquidity pool if we're currently on the edge of
    /// one
    pub fn buy_to_price(&self, target_price: SqrtPriceX96) -> Option<PriceRange<'a>> {
        self.order_to_target(Some(target_price), true)
    }

    /// Build a PriceRange from the current price to the bound of the current liquidity pool.  Will jump to
    /// the next liquidity pool if we're currently on the edge of one.
    ...
    /// This will produce a Uniswap Price Range that spans from the current price to the CLOSER of the target
    /// price or the nearest liquidity pool boundary
    fn order_to_target(&self, target_price: Option<SqrtPriceX96>, buy: bool) -> Option<PriceRange<'a>> {
        // Bounds check our target price if provided
        if let Some(p) = target_price {
            if buy {
                // Buying from the market will raise the price, so if our target price is on the wrong side of our
                // current price, we can't do this.
...
impl <'a> PriceRange<'a> {
    pub fn quantity(&self, target_price: f64) -> f64 {
        ...
        if self.end_bound.price > self.start_bound.price {
            if t > self.end_bound.price { return self.quantity.into(); }
        } else {
            if t < self.end_bound.price { return self.quantity.into(); }
        }
...
mod tests {
    ...
    fn span_sums_and_rounding_work() {
        ...
        let step_12 = uniswap_v3_math::sqrt_price_math::_get_amount_0_delta(t1, t2, liq, true).unwrap();
        let step_23 = uniswap_v3_math::sqrt_price_math::_get_amount_0_delta(t2, t3, liq, true).unwrap();
        let step_13 = uniswap_v3_math::sqrt_price_math::_get_amount_0_delta(t1, t3, liq, true).unwrap();
