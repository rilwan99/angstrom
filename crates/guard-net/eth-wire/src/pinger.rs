...
/// The pinger is a state machine that is created with a maximum number of pongs
/// that can be missed.
#[derive(Debug)]
...
impl Pinger {
    ...
    /// Mark a pong as received, and transition the pinger to the `Ready` state
    /// if it was in the `WaitingForPong` state. Unsets the sleep timer.
    ...
    /// Polls the state of the pinger and returns whether a new ping needs to be
    /// sent or if a previous ping timed out.
    pub(crate) fn poll_ping(
        ...
        match self.state() {
            ...
            PingState::TimedOut => {
                // we treat continuous calls while in timeout as pending, since the connection
...
pub(crate) enum PingState {
    /// There are no pings in flight, or all pings have been responded to, and
    /// we are ready to send a ping at a later point.
    Ready,
    /// We have sent a ping and are waiting for a pong, but the peer has missed
    /// n pongs.
    WaitingForPong,
...
mod tests {
    ...
    async fn test_ping_timeout() {
        ...
        // we should wait for the interval to elapse and receive a pong before the
        // timeout elapses
