...
/// Obtains a PeerId from an ENR. In this case, the PeerId represents the public
/// key contained in the ENR.
pub fn enr_to_peer_id(enr: Enr<SigningKey>) -> PeerId {
    // In the following tests, methods which accept a public key expect it to
    // contain the public key in its 64-byte encoded (uncompressed) form.
...
/// Creates two unused SocketAddrs, intended for use as the p2p (TCP) and
/// discovery ports (UDP) for new reth instances.
