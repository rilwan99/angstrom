...
pub trait BlockImport: std::fmt::Debug + Send + Sync {
    ...
    ///
    /// > When a `NewBlock` announcement message is received from a peer, the
    /// > client first verifies
    /// > the basic header validity of the block, checking whether the
    /// > proof-of-work value is valid.
    ///
    /// This is supposed to start verification. The results are then expected to
    /// be returned via [`BlockImport::poll`].
...
pub enum BlockValidation {
    /// Basic Header validity check, after which the block should be relayed to
    /// peers via a `NewBlock` message
    ValidHeader {
        /// received block
        block: NewBlockMessage
    },
    /// Successfully imported: state-root matches after execution. The block
    /// should be relayed via `NewBlockHashes`
    ValidBlock {
        /// validated block.
        block: NewBlockMessage
    }
...
/// An implementation of `BlockImport` used in Proof-of-Stake consensus that
/// does nothing.
///
