...
//!
//! Discv4 employs a kademlia-like routing table to store and manage discovered
//! peers and topics. The protocol allows for external IP discovery in NAT
//! environments through regular PING/PONG's with discovered nodes. Nodes return
//! the external IP address that they have received and a simple majority is
//! chosen as our external IP address. If an external IP address is updated,
//! this is produced as an event to notify the swarm (if one is used for this
//! behaviour).
//!
//! This implementation consists of a [`Discv4`] and [`Discv4Service`] pair. The
//! service manages the state and drives the UDP socket. The (optional)
//! [`Discv4`] serves as the frontend to interact with the service via a
//! channel. Whenever the underlying table changes service produces a
...
/// Length of the UDP datagram packet-header: Hash(32b) + Signature(65b) +
/// Packet Type(1b)
...
/// Maximum number of nodes to ping at concurrently. 2 full `Neighbours`
/// responses with 16 _new_ nodes. This will apply some backpressure in
/// recursive lookups.
const MAX_NODES_PING: usize = 2 * MAX_NODES_PER_BUCKET;

/// The size of the datagram is limited [`MAX_PACKET_SIZE`], 16 nodes, as the
/// discv4 specifies don't fit in one datagram. The safe number of nodes that
/// always fit in a datagram is 12, with worst case all of them being IPv6
/// nodes. This is calculated by `(MAX_PACKET_SIZE - (header + expire +
/// rlp overhead) / size(rlp(Node_IPv6))`
/// Even in the best case where all nodes are IPv4, only 14 nodes fit into one
/// packet.
...
impl Discv4 {
    ...
    /// Sets the [Interval] used for periodically looking up targets over the
    /// network
    ...
    ///
    /// The lookup initiator starts by picking α closest nodes to the target it
    /// knows of. The initiator then sends concurrent FindNode packets to
    /// those nodes. α is a system-wide concurrency parameter, such as 3. In
    /// the recursive step, the initiator resends FindNode to nodes it has
    /// learned about from previous queries. Of the k nodes the initiator has
    /// heard of closest to the target, it picks α that it has not yet
    /// queried and resends FindNode to them. Nodes that fail to respond
    /// quickly are removed from consideration until and unless they do
    /// respond.
    //
    // If a round of FindNode queries fails to return a node any closer than the
    // closest already seen, the initiator resends the find node to all of the k
    // closest nodes it has not already queried. The lookup terminates when the
    ...
    /// Returns the receiver half of new listener channel that streams
    /// [`DiscoveryUpdate`]s.
...
pub struct Discv4Service {
    ...
    /// A cap on concurrent `Ping` prevents escalation where: A large number of
    /// new nodes discovered via `FindNode` in a recursive lookup triggers a
    /// large number of `Ping`s, and followup `FindNode` requests....
...
impl Discv4Service {
    ...
    pub(crate) fn new(
        ...
        // Wait `ping_interval` and then start pinging every `ping_interval` because we
        // want to wait for
    ...
    /// Returns the frontend handle that can communicate with the service via
    /// commands.
    ...
    /// Sets the given ip address as the node's external IP in the node record
    /// announced in discovery
    ...
    /// closest proven neighbours.
    ///
    /// This is similar to adding all bootnodes via [`Self::add_node`], but does
    /// not fire a [`DiscoveryUpdate::Added`] event for the given bootnodes.
    /// So boot nodes don't appear in the update stream, which is usually
    /// desirable, since bootnodes should not be connected to.
    ///
    /// If adding the configured boodnodes should result in a
    /// [`DiscoveryUpdate::Added`], see [`Self::add_all_nodes`].
    ///
    ...
    ///
    /// A FindNode packet requests information about nodes close to target. The
    /// target is a 64-byte secp256k1 public key. When FindNode is received,
    /// the recipient should reply with Neighbors packets containing the
    /// closest 16 nodes to target found in its local table.
    //
    // To guard against traffic amplification attacks, Neighbors replies should only
    // be sent if the sender of FindNode has been verified by the endpoint proof
    // procedure.
    ...
    ///
    /// At first the `ALPHA` (==3, defined concurrency factor) nodes that are
    /// closest to the target in the underlying DHT are selected to seed the
    /// lookup via `FindNode` requests. In the recursive step, the initiator
    /// resends FindNode to nodes it has learned about from previous
    /// queries.
    ///
    /// This takes an optional Sender through which all successfully discovered
    /// nodes are sent once the request has finished.
    fn lookup_with(&mut self, target: PeerId, tx: Option<NodeRecordSender>) {
        ...
        if closest.is_empty() && self.pending_find_nodes.is_empty() {
            ...
            // This could happen if all records were deleted from the table due to missed
            // pongs (e.g. connectivity problems over a long period of time, or
            // issues during initial bootstrapping) so we attempt to bootstrap
            // again
            self.bootstrap();
            return
        }
    ...
    /// Sends a new `FindNode` packet to the node with `target` as the lookup
    /// target.
    ///
    /// CAUTION: This expects there's a valid Endpoint proof to the given
    /// `node`.
    fn find_node(&mut self, node: &NodeRecord, ctx: LookupContext) {
        ...
        self.pending_find_nodes
            .insert(node.id, FindNodeRequest::new(ctx));
    }

    /// Notifies all listeners
    fn notify(&mut self, update: DiscoveryUpdate) {
        self.update_listeners
            .retain_mut(|listener| match listener.try_send(update.clone()) {
                Ok(()) => true,
                Err(err) => match err {
    ...
    ///
    /// This allows applications, for whatever reason, to remove nodes from the
    /// local routing table. Returns `true` if the node was in the table and
    /// `false` otherwise.
    ...
    pub fn num_connected(&self) -> usize {
        self.kbuckets
            .buckets_iter()
            .fold(0, |count, bucket| count + bucket.num_connected())
    }
    ...
    ///
    /// On re-ping we check for a changed enr_seq if eip868 is enabled and when
    /// it changed we sent a followup request to retrieve the updated ENR
    ...
    fn update_on_pong(&mut self, record: NodeRecord, mut last_enr_seq: Option<u64>) {
        ...
        // if the peer included a enr seq in the pong then we can try to request the ENR
        // of that node
    ...
    /// If the node's not in the table yet, this will add it to the table and
    /// start the endpoint proof by sending a ping to the node.
    ///
    /// Returns `true` if the record was added successfully, and `false` if the
    /// node is either already in the table or the record's bucket is full.
    ...
    fn on_ping(&mut self, ping: Ping, remote_addr: SocketAddr, remote_id: PeerId, hash: H256) {
        ...
        // > If no communication with the sender of this ping has occurred within the
        // > last 12h, a
        // > ping should be sent in addition to pong in order to receive an endpoint
        // > proof.
        //
        // Note: we only mark if the node is absent because the `last 12h` condition is
        // handled by the ping interval
            ...
            kbucket::Entry::Absent(entry) => {
                ...
                match entry.insert(
                    ...
                    BucketInsertResult::TooManyIncoming | BucketInsertResult::NodeExists => {
                        needs_bond = true;
                        // insert unsuccessful but we still want to send the
                        // pong
                    }
        ...
        // send the pong first, but the PONG and optionally PING don't need to be send
        ...
        // if node was absent also send a ping to establish the endpoint proof from our
        // end
        if is_new_insert {
            self.try_ping(record, PingReason::InitialInsert);
        } else if needs_bond {
    ...
    /// Handler for incoming `Neighbours` messages that are handled if they're
    /// responses to `FindNode` requests.
    fn on_neighbours(&mut self, msg: Neighbours, remote_addr: SocketAddr, node_id: PeerId) {
        ...
        // This is the recursive lookup step where we initiate new FindNode requests for
        // new nodes that where discovered.
    ...
    fn evict_failed_neighbours(&mut self, now: Instant) {
        ...
        self.pending_find_nodes
            .retain(|node_id, find_node_request| {
                if now.duration_since(find_node_request.sent_at) > self.config.request_timeout {
                    if !find_node_request.answered {
                        // node actually responded but with fewer entries than expected, but we
                        // don't treat this as an hard error since it
                        // responded.
        ...
        for node_id in failed_neighbours {
            ...
            // if the node failed to respond anything useful multiple times, remove the node
            // from the table, but only if there are enough other nodes in the
    ...
    /// Re-pings all nodes which endpoint proofs are considered expired:
    /// [``NodeEntry::is_expired]
    ///
    /// This will send a `Ping` to the nodes, if a node fails to respond with a
    /// `Pong` to renew the endpoint proof it will be removed from the
    /// table.
    ...
    ///
    /// Note: this accepts the timestamp as u64 because this is used by the wire
    /// protocol, but the UNIX timestamp (number of non-leap seconds since
    /// January 1, 1970 0:00:00 UTC) is supposed to be an i64.
    ///
    /// Returns an error if:
    ...
    ///
    /// To prevent traffic amplification attacks, implementations must verify
    /// that the sender of a query participates in the discovery protocol.
    /// The sender of a packet is considered verified if it has sent a valid
    /// Pong response with matching ping hash within the last 12 hours.
    pub(crate) fn poll(&mut self, cx: &mut Context<'_>) -> Poll<Discv4Event> {
        loop {
            ...
            if let Some(Poll::Ready(Some(ip))) = self
                .resolve_external_ip_interval
                .as_mut()
                .map(|r| r.poll_tick(cx))
            {
            ...
            while let Poll::Ready(Some(cmd)) = self.commands_rx.poll_recv(cx) {
                match cmd {
                    ...
                    Discv4Command::SetEIP868RLPPair { key, rlp } => {
                        ...
                        let _ = self
                            .local_eip_868_enr
                            .insert_raw_rlp(key, rlp, &self.secret_key);
                    }
            ...
            while self.expire_interval.poll_tick(cx).is_ready() {
                self.received_pongs
                    .evict_expired(Instant::now(), EXPIRE_DURATION)
            }
...
///
/// This is mainly used for testing purposes and represents messages the service
/// processed
...
/// Continuously reads new messages from the channel and writes them to the
/// socket
...
/// Continuously awaits new incoming messages and sends them back through the
/// channel.
...
///
/// By selecting different targets, the lookups will be seeded with different
/// ALPHA seed nodes.
#[derive(Debug)]
...
// SAFETY: The [`Discv4Service`] is intended to be spawned as task which
// requires `Send`. The `LookupContext` is shared by all active `FindNode`
// requests that are part of the lookup step. Which can modify the context. The
// shared context is only ever accessed mutably when a `Neighbour` response is
// processed and all Clones are stored inside [`Discv4Service`], in other words
// it is guaranteed that there's only 1 owner ([`Discv4Service`]) of all
// possible [`Rc`] clones of [`LookupContext`].
...
impl Drop for LookupContextInner {
    fn drop(&mut self) {
        if let Some(tx) = self.listener.take() {
            // there's only 1 instance shared across `FindNode` requests, if this is dropped
            // then all requests finished, and we can send all results back
            let nodes = self
                .closest_nodes
                .take()
                .into_values()
...
enum PingReason {
    /// Initial ping to a previously unknown peer that was inserted into the
    /// table.
    InitialInsert,
    /// Initial ping to a previously unknown peer that didn't fit into the
    /// table. But we still want to establish a bond.
    EstablishBond,
    /// Re-ping a peer..
    RePing,
...
pub enum DiscoveryUpdate {
    ...
    Added(NodeRecord),
    /// A new node was discovered but _not_ added to the table because it is
    /// currently full.
    ...
    Removed(PeerId),
    /// A series of updates
    Bundle(Vec<DiscoveryUpdate>)
}

/// Represents a forward-compatible ENR entry for including the forkid in a node
/// record via EIP-868. Forward compatibility is achieved by allowing trailing
/// fields.
///
/// See:
...
mod tests {
    ...
    async fn test_check_wrong_to() {
        ...
        service_1
            .pending_pings
            .insert(*service_2.local_peer_id(), ping_request);
    ...
    async fn test_check_ping_pong() {
        ...
        // Since the endpoint was already proven from 1 POV it can already send a
        // FindNode so the next event is either the PONG or Find Node
        match event {
